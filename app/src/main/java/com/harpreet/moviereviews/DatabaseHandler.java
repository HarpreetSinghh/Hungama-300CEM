...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    // these are all the variables stored
    // Contacts table name
    // Defining my table columns
    ...
    //with oncreate method we create a database
    @Override
    ...
    // Here you use the column names; these specify the columns which will be inserted.
            // if these are misspelled compared to what is in the database then inserting will fail.
            // every column is specified here but not the ID which can be automatically created as a Unique ID.
    {
        ...
        values.put(KEY_MOB_NO, registerdata.getMobNo());//register username
    ...
    String getregister(String username) {
        ...
        // a Cursor leaves a lot to be desired as a way you look at the table, this gives you raw column values. pulling data out of a cursor

        Cursor cursor = db.query(TABLE_REGISTER, null, "mobile_number=?", new String[]{username}, null, null, null, null);
    ...
    String isEmail(String email) {
        ...
        if (cursor.getCount() < 1) {
            ...
            password = cursor.getString(cursor.getColumnIndex(KEY_EMAIL_ID));
            cursor.close();
